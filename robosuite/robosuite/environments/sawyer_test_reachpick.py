...
class SawyerReachPick(SawyerEnv):
    """
    ...
    def __init__(
        ...
        # order: di["eef_pos"] di["gripper_qpos"] di["gripper_to_marker"]
        # dim: 3, 2, 3, 3
    ...
    def _reset_internal(self):
        ...
        self.goal = np.array(self.sim.data.body_xpos[self.cube_body_id]) + np.array((0.0, 0.0, 0.075))
    ...
    def reward(self, action=None):
        ...
        if self.reward_shaping:
            ...
            # gripper to block incentive (0 to 1)
            eef_to_block_dist = np.linalg.norm(gripper_site_pos - (cube_pos + np.array((0.0, 0.0, 0.03))))
            cube_to_goal_dist = np.linalg.norm(self.goal - cube_pos)
            # slightly above block
            reward = reward - eef_to_block_dist - cube_to_goal_dist
            # print("reach reward: {}".format(-1 * eef_to_block_dist))
            ...
            if touch_left_finger and touch_right_finger \
                    and eef_to_block_dist < self.threshold \
                    and cube_to_goal_dist < self.threshold:
                reward = 10.0



        else:
            eef_to_block_dist = np.linalg.norm(gripper_site_pos - cube_pos)
            cube_to_goal_dist = np.linalg.norm(self.goal - cube_pos)
            # if eef_to_block_dist < self.threshold:
            #     reward = reward + 1.0
            #
            # # grasping incentive (0 or 0.25)
            touch_left_finger = False
            touch_right_finger = False
            for i in range(self.sim.data.ncon):
                c = self.sim.data.contact[i]
    ...
    # for goalenv wrapper, does not work for HER in current state
    ...
    def _get_observation(self):
        ...
        if self.use_object_obs:
            ...
            # )
            di["object-state"] = np.concatenate(
                [di["gripper_to_cube"], cube_pos, self.goal]
            )
